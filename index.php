...
if (!isset($_SESSION['id_user']) || !isset($_SESSION['posisi'])) {
    echo "<script>
...
$posisi = $_SESSION['posisi']; // Mendapatkan posisi pengguna dari session
?>
...
<head>
    ...
    <script>
        // JavaScript untuk membuka modal secara otomatis
        $(document).ready(function() {
            <?php if ($posisi == 'Pimpinan' || $posisi == 'Admin'): ?>
                $('#Modal').modal('show'); // Otomatis membuka modal
            <?php endif; ?>
        });
    </script>
...
<body id="page-top" data-login-status="success">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content" style="background-color:#ffefec">
                ...
                <div class="container-fluid">
                        ...
                        <div class="col">
                            <div class="row justify-content-center">
                                <div class="card border-0" style="background-color:#ffefec">
                                    <div class="row justify-content-center">
                                        <a href="index.php" class="btn btn-sm text-white" style="background-color:#ff9a8aff; font-size: 14px">Tahunan</a>
                                        <a href="index-bulanan.php" class="btn btn-sm" style="background-color:#fff; font-size:14px">Bulanan</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- ======================================================================== Content Row - Card Earnings ======================================================================== -->
                    
                    <div class="row" style="margin-top:1rem">

                        <!-- Total Events Semua Data -->
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="h5 mb-0 font-weight-bold" style="margin-top:7px">
                                                <?php
                                                $totalevents = mysqli_query($conn, "SELECT * FROM `tb_event`");
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="h5 mb-0 font-weight-bold" style="margin-top:7px">
                                                <?php
                                                $totalclients = mysqli_query($conn, "SELECT DISTINCT nama_client FROM `tb_event`");
                                            ...
                                            <div class="row no-gutters align-items-center">
                                                <div class="col-auto">
                                                    <div class="h5 mb-0 mr-3 font-weight-bold" style="margin-top:7px">
                                                        <?php
                                                        $totallocations = mysqli_query($conn, "SELECT DISTINCT `lokasi_event` FROM `tb_event`");
                                                        echo mysqli_num_rows($totallocations);
                                                        ?>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <!-- Bar Chart & Update Event -->
                    <div class="row">
                        <!-- Bar Chart Semua Data -->
                        <div class="col-xl-8 col-lg-7">
                            <div class="card border-0 mb-4">
                                <?php
                                // Query untuk mendapatkan tahun terbaru di database
                                $queryMaxYear = "SELECT MAX(YEAR(hari_tgl)) as max_year FROM tb_event";
                                $resultMaxYear = mysqli_query($conn, $queryMaxYear);
                                $rowMaxYear = mysqli_fetch_assoc($resultMaxYear);
                                $maxYear = $rowMaxYear['max_year'];
                                // Dapatkan 5 tahun terakhir
                                $minYear = $maxYear - 4; // Menghitung 5 tahun terakhir
                                // Query untuk mendapatkan data hanya 5 tahun terakhir
                                $queryYears = "SELECT DISTINCT YEAR(hari_tgl) as year FROM tb_event WHERE YEAR(hari_tgl) BETWEEN '$minYear' AND '$maxYear' ORDER BY year ASC";
                                ...
                                // Fungsi untuk mendapatkan total data event per tahun
                                function getDataByYear($conn, $year)
                                {
                                    $query = "SELECT * FROM `tb_event` WHERE YEAR(hari_tgl) = '$year';";
                                    $result = mysqli_query($conn, $query);
                                    return mysqli_num_rows($result);
                                }
                                // Data tahunan hanya untuk 5 tahun terakhir
                                $yearlyData = [];
                                foreach ($years as $year) {
                                    $yearlyData[] = [
                                        'year' => $year,
                                        'total' => getDataByYear($conn, $year)
                                    ];
                                }
                                // Warna khusus untuk tiap tahun, ulangi jika melebihi tahun 2026
                                $colors = ["#ff9a8aff", "#a1ccd1", "#b5cda3", "#eec373", "#f46060"];
                                $hoverColors = ["#ff7878", "#89bbbb", "#a0bd94", "#d7a863", "#e45353"];
                                ?>
                                <!-- Card Body -->
                    ...
                    <!-- Best Location, Doughnut Chart & Rating -->
                    <div class="row">
                        <!-- Best Locations Semua Data -->
                        <div class="col">
                            <div class="card border-0 mb-4">
                                <div class="card-header">
                                ...
                                <div class="card-body" style="height:415px; padding:0px 0px 0px 0.5px" id="overflowTest">
                                    <table class="table">
                                        ...
                                        <tbody class="small">
                                            <?php
                                            $no = 1;
                                            $query = "SELECT COUNT(*) AS total, lokasi_event, arealokasi_event FROM tb_event GROUP BY lokasi_event, arealokasi_event ORDER BY total DESC;";
                                            $sql = mysqli_query($conn, $query);
                                            while ($row = mysqli_fetch_assoc($sql)) {
                                            ?>
                                                <tr>
                            ...
                            <div class="card border-0 mb-4 text-center">
                                ...
                                <div class="card-body" style="height: 422px; align-items: center; padding:0;">
                                    ...
                                    <div class="row justify-content-center" style="margin-top: 1rem">
                                        ...
                                        (COUNT(penilaian_satu) + COUNT(penilaian_dua) + COUNT(penilaian_tiga) + COUNT(penilaian_empat) + COUNT(penilaian_lima)) AS total FROM tb_rating;";
                                            $sql = mysqli_query($conn, $query);
                                            while ($row = mysqli_fetch_assoc($sql)) {
                                    ...
                                    <div class="row justify-content-center align-items-center" style="margin-bottom: 1rem;">
                                        ...
                                        <span style="color:#FF7878; font-weight:bold; margin-right:3px;">
                                            <?php
                                            $query = "SELECT COUNT(*) AS totalUserReview FROM tb_rating;";
                                            $sql = mysqli_query($conn, $query);
                                    ...
                                    <div class="row" style="margin:auto">
                                        ...
                                        <div>
                                            ...
                                            $query = "SELECT AVG(penilaian_satu) AS komunikasi FROM tb_rating";
                                        ...
                                        <div>
                                            ...
                                            $query = "SELECT AVG(penilaian_dua) AS perencanaan FROM tb_rating";
                                            $result = $conn->query($query);
                                            // Cek apakah ada hasil
                                            if ($result->num_rows > 0) {
                                                // Ambil nilai rating
                                        ...
                                        <div>
                                            ...
                                            $query = "SELECT AVG(penilaian_tiga) AS pelaksanaan FROM tb_rating";
                                            $result = $conn->query($query);
                                            // Cek apakah ada hasil
                                            if ($result->num_rows > 0) {
                                                // Ambil nilai rating
                                        ...
                                        <div>
                                            ...
                                            $query = "SELECT AVG(penilaian_empat) AS kepuasan FROM tb_rating";
                                            $result = $conn->query($query);
                                            // Cek apakah ada hasil
                                            if ($result->num_rows > 0) {
                                                // Ambil nilai rating
                                        ...
                                        <div>
                                            ...
                                            $query = "SELECT AVG(penilaian_lima) AS rekomendasi FROM tb_rating";
                                            $result = $conn->query($query);
                                            // Cek apakah ada hasil
                                            if ($result->num_rows > 0) {
                                                // Ambil nilai rating
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Fungsi untuk menutup sidebar pada perangkat mobile
        function closeSidebarOnMobile() {
            if (window.innerWidth <= 768) { // Deteksi ukuran layar mobile
                closeSidebar();
            }
        }

        // Panggil fungsi saat halaman dimuat atau di-reload
        closeSidebarOnMobile();

        // Event listener untuk menutup sidebar saat ukuran layar berubah
        window.addEventListener("resize", closeSidebarOnMobile);

        // Simulasikan login berhasil dan tutup sidebar
        function onLoginSuccess() {
            // Contoh: deteksi keberhasilan login
            const loginStatus = document.body.dataset.loginStatus; // Ganti sesuai cara Anda mendeteksi status login
            if (loginStatus === "success") {
                closeSidebar(); // Tutup sidebar jika login berhasil
            }
        }

        // Jalankan fungsi untuk login berhasil
        onLoginSuccess();
    });
</script>
    ...
    <script>
        ...
        // Array warna yang akan diulang jika melebihi 5 tahun
        var backgroundColors = [];
        var hoverBackgroundColors = [];

        <?php
        // Mengisi warna untuk setiap tahun menggunakan perulangan warna
        $i = 0;
        foreach ($yearlyData as $data) {
            // Pilih warna berdasarkan index
            $colorIndex = $i % count($colors);
            echo "backgroundColors.push('" . $colors[$colorIndex] . "');";
            echo "hoverBackgroundColors.push('" . $hoverColors[$colorIndex] . "');";
            $i++;
        }
        ?>
        ...
        var myBarChart = new Chart(ctx, {
            ...
            data: {
                labels: [
                    <?php
                    // Loop untuk menampilkan label tahun secara dinamis di chart
                    foreach ($yearlyData as $data) {
                        echo "'" . $data['year'] . "',";
                    }
                    ?>
                ...
                datasets: [{
                    label: "Total Data Tahunan",
                    backgroundColor: backgroundColors, // Warna dinamis
                    hoverBackgroundColor: hoverBackgroundColors, // Warna saat di-hover
                    data: [
                        <?php
                        // Loop untuk menampilkan total data per tahun di chart
                        foreach ($yearlyData as $data) {
                            echo $data['total'] . ",";
                        }
                        ?>
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Data Tahunan (5 Tahun Terakhir)'
                    }
                }
            },
        });
    ...
    <script>
        <?php
        // Ambil tahun terbaru yang ada di database
        $queryLatestYear = mysqli_query($conn, "SELECT MAX(YEAR(hari_tgl)) as latest_year FROM `tb_event`;");
        $rowLatestYear = mysqli_fetch_assoc($queryLatestYear);
        $latestYear = $rowLatestYear['latest_year']; // Tahun terbaru di database
        // Hitung batas tahun terendah untuk rentang 5 tahun terakhir
        $startYear = $latestYear - 5;
        // Ambil lokasi unik dari event yang terjadi dalam 5 tahun terakhir dari tahun terbaru
        $queryLocations = mysqli_query($conn, "SELECT DISTINCT arealokasi_event FROM `tb_event` WHERE YEAR(hari_tgl) BETWEEN $startYear AND $latestYear ORDER BY arealokasi_event ASC;");
        // Array untuk menyimpan lokasi (xValues) dan jumlah event (yValues)
        $locations = [];
        $eventCounts = [];
        // Loop untuk mendapatkan setiap lokasi unik dan menghitung jumlah event di tiap lokasi dalam rentang 5 tahun terakhir
        while ($row = mysqli_fetch_assoc($queryLocations)) {
            $formattedLocation = ucwords(strtolower($row['arealokasi_event'])); // Ubah format nama lokasi menjadi besar-kecil
            $locations[] = $formattedLocation;
            // Hitung jumlah event berdasarkan lokasi dalam rentang 5 tahun terakhir
            $location = $row['arealokasi_event'];
            $queryCount = mysqli_query($conn, "SELECT * FROM `tb_event` WHERE arealokasi_event = '$location' AND YEAR(hari_tgl) BETWEEN $startYear AND $latestYear;");
            $eventCounts[] = mysqli_num_rows($queryCount);
        }
        // Konversi array PHP ke format JSON untuk digunakan di JavaScript
        $xValues = json_encode($locations); // Nama lokasi
        $yValues = json_encode($eventCounts); // Jumlah event
        ?>
